...
```{r}
my.tree <- TreeSim::sim.bd.taxa(n=30, numbsim=1, lambda=0.1, mu=0)[[1]] #simulation is random
```
...
```{r}
plot(my.tree) #hmm ok...
#there are other ways to visualize trees. How about lineage through time plots! Showing the relationship between simulated Nt and time. 
```

One way to look at trees, and actually what many methods reduce to under the hood, is lineage through time plots. 

```{r}
ape::ltt.plot(my.tree) #negative time? moving brom the tips (right hand side) to ancestral nodes (left hand side)
```
...
```{r}
bd.trees <- TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=.9, complete=FALSE) #lambda:speciation #mu:extinction
...
Comparing.trees<-function(depth.range,max.depth,yule.trees,bd.trees){
...
legend("topleft", legend=c("Model 1", "Model 2"), fill=colors)
}  
Comparing.trees(depth.range,max.depth,yule.trees,bd.trees)
...
depth.range <- range(unlist(lapply(yule.trees,ape::branching.times)), unlist(lapply(bd.trees,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
plot(x=c(0, -4), y=c(20, ape::Ntip(yule.trees[[1]])), log="y", type="n", bty="n", xlab="Time", ylab="N") #something off with the axes and the scaling bit. 
colors=c(rgb(1,0,0,0.5), rgb(0, 0, 0, 0.5))
list.of.both <- list(bd.trees, yule.trees)
for (i in sequence(2)) {
    tree.list <- list.of.both[[i]]
...
legend("topleft", legend=c("Model 1", "Model 2"), fill=colors) #birth death model, in the zoomed in portion is just the birth rate 
```
...
**TODO** Try plotting some with other diversification parameters. What happens if speciation rate is much higher than extinction rate? How does the simulation change with different values, but keeping their difference constant? If their sum is constant? 

Changing speciation rate holding mu constant. 
```{r} 
#run some simmies and plot them, understand patterns of branch length change over time
yule.1<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=.2, mu=0, complete=FALSE) #model 1
yule.2<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=0, complete=FALSE) #model 2
depth.range <- range(unlist(lapply(yule.1,ape::branching.times)), unlist(lapply(yule.2,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
Comparing.trees(depth.range,max.depth,yule.1,yule.2)
```
Speciation rate higher than extinction rate, varying extinction rates. 
```{r}
bd.1<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=0, complete=FALSE) #model 1
bd.2<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=.8, complete=FALSE) #model 2
depth.range <- range(unlist(lapply(bd.1,ape::branching.times)), unlist(lapply(bd.2,ape::branching.times)))
max.depth <- sum(abs(depth.range))
Comparing.trees(depth.range,max.depth,bd.1,bd.2)
```
Holding difference between parameters constant
```{r}
bd.3<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=.8, complete=FALSE) #model 1
bd.4<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=.2, mu=.0, complete=FALSE) #model 2
depth.range <- range(unlist(lapply(bd.3,ape::branching.times)), unlist(lapply(bd.4,ape::branching.times)))
max.depth <- sum(abs(depth.range))
Comparing.trees(depth.range,max.depth,bd.3,bd.4)
```
Holding sums contant between parameters (tau)
```{r}
bd.5<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=.8, mu=.2, complete=FALSE) #model 1
bd.6<-TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=.7, mu=.3, complete=FALSE) #model 2
depth.range <- range(unlist(lapply(bd.5,ape::branching.times)), unlist(lapply(bd.6,ape::branching.times)))
max.depth <- sum(abs(depth.range))
Comparing.trees(depth.range,max.depth,bd.5,bd.6)
```
...
extinction.rates <- rep(0.03, 4) #same extinction rate
transition.rates <- c(0.01,0.01,0, 0.01, 0, 0.01, 0.01,0,0.01, 0,0.01,0.01) #0A->1A, 0A->0B, 0A->1B, 1A->0A, 1A->0B, 1A->1B, 0B->0A, 0B->1A, 0B->1B, 1B->0A, 1B->1A, 1B->0B #for actual observed transiton rates other than 0 (correlated traits), they are held constant. 
...
```{r, eval=TRUE}
turnover.anc = c(1,1,0,0) #tau #net diversification rate
eps.anc = c(1,1,0,0) #extinction fraction 
```
...
```{r, eval=TRUE}
#githubinstall::gh_install_packages(packages = "https://github.com/thej022214/hisse/tree/master/vignettes")
load("testrecon1.Rsave") #where is this file? #googled, found on github, downloaded. 
class(pp.recon)
pp.recon
```
...
```{r, eval=TRUE}
pp.recon$aic
pp$AIC #why would I want to use this over pp.recon$aic?
```
...
pp.recon = MarginRecon(phy, sim.dat, f=c(1,1), hidden.states=TRUE, pars=pp$solution, 
                       aic=pp$aic, n.cores=2) #oh I see, we are supplanting the aic from pp to make pp.recon match our intial hisse model. 
```
...
for(i in sequence(length(files))){
  ...
  rm(pp.recon)
} #alright, I could try to do so with my own data. 
```

## Your data
```{r}
library(corHMM)
tree<-read.nexus(file = "EFN_tree.nex") #EFN in legume
dat<-read.nexus.data(file="EFN_dat.nex") 
tree<-drop.random(tree,700)
CleanData <- function(phy, data) {
	treedata(phy,data)
}
devtools::install_github("thej022214/corHMM", dependencies = FALSE)
df<- data.frame(taxon=names(dat), trait=unlist(dat), stringsAsFactors = FALSE)
dat<-df
newnew<-CleanData(tree,dat)
tree<-newnew$phy
dat<-newnew$data
#estimating fixed parameters #hisse can do this too. 
corHMM_output<-corHMM(tree,df,rate.cat=2,node.states="none") #transition rates
bdefn<-birthdeath(tree) #speciation and extinction

#optimizing parameters
#eps
epsilon<-bdefn$para[1] #hisse can output this 
epsilon<-rep(epsilon,4)
epsilon<-rep(1,4)
#tau is b+d not b-d (net. div)
d<-(bdefn$para[1]*bdefn$para[2])/(1-bdefn$para[1])
b<-bdefn$para[2]+d
tau<-b+d
tau<-rep(tau,4)
tau<-unname(tau) #fixed #hisse can output this
tau<-rep(1,4) #optomized
#trans
transition.rates<-corHMM_output$solution 
transition.rates[which(is.na(transition.rates))]<-0
diag(transition.rates)<-NA
transition.rates<-ParDrop(trans.rates, c(3,5,8,10))
#null
...
#null
null.hiss<- hisse(tree,dat, hidden.states=TRUE, turnover.anc=c(1,1,1,1), eps.anc=c(1,1,1,1), trans.rate=trans.rates.hisse) #all things equal

#alt 1
pp.efn<-hisse(tree, dat, hidden.states=TRUE, turnover.anc=tau, eps.anc=epsilon, trans.rate=transition.rates) 
#tau all equal, transition rates differ, eps all equal

#alt 2
pp.efn2<-hisse(tree, dat, hidden.states=TRUE, turnover.anc=c(1,2,3,4), eps.anc=c(1,2,3,4), trans.rate=transition.rates) 
#tau differ, transition rates differ, epsilon differ
#compare aic scores here, choose best model to plot #pp.efn2 most complex, however most complex (maybe more realistic to actual biological context)

#alt 3 
pp.efn3<-hisse(tree, dat, hidden.states=TRUE, turnover.anc=c(1,1,2,2), eps.anc=c(1,1,2,2), trans.rate=trans.rates.nodual.threerates)

aic.comp<-c(null.hiss$AIC,pp.efn$AIC,pp.efn2$AIC,pp.efn3$AIC)
aicc.comp<-c(null.hiss$AICc,pp.efn$AICc,pp.efn2$AICc,pp.efn3$AIC)

pp.marg<-MarginRecon(tree, dat, f=c(.5,.5), hidden.states=TRUE, pars=pp.efn2$solution, aic=pp.efn2$aic, n.cores=2)
#what does n.cores=2 really imply? argument explantaion seems vague. 
#runs slow, prunned tree for this exercise. 

plot.hisse.states(pp.marg, rate.param="net.div", show.tip.label=FALSE) #intial plot #stunning
```

```{r}
plot.hisse.states(pp.marg, rate.param="net.div", show.tip.label=FALSE, rate.range=c(0,0.5)) #a beaut!
```
