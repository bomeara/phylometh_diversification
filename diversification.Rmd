...
```{r global_options, eval=T,include=FALSE}
...
## Your data
As before, I will be wokring with bacteria and whether or not they have the Type-III secretion system. I will compare the HISSE model to the BISSE model to determine if these traits might be tied to a hidden trait. To start off with, I will assume a pure birth process for HISSE/BISSE and assume a single diversification rate.
```{r,eval=T}
...
Now with a shared extinction rates 
```{r,eval=T}
turnover.anc = c(1,2,3,4) ## All states have free parameters
eps.anc = c(1,1,1,1)
...
Finally, determine the best model using AICc. 
...
According to this, the best model appears to be the HISSE model, with no extinction and transition rates not equal. However, I interpret these results with caution, given the warnings when extinction rates were included in the model. The hidden state which could be driving differences in diversification rates could be whether or not the bacteria is pathogenic. 