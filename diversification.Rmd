...
mltt.plot(yule.trees, xlab = "Time", ylab = "N")
...
```{r, eval=TRUE}
birthdeath.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=1, mu=0.9, complete=FALSE)
ape::mltt.plot(bd.trees, log="y", legend=FALSE)

my.trees1 <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=0.3, mu=0, complete=FALSE)
...
```{r, eval=TRUE}
pp.recon <- MarginRecon(phy, sim.dat, f=c(1,1), hidden.states=TRUE, pars=pp$solution, aic=pp$aic, n.cores=1)
```
...
files.to.load <- list.files(pattern="Rsave")
files<- for (i in sequence(length(files.to.load))) {
  load(files.to.load[i])
...
## Your data
My question to consider now: Does seed size matter? I am using the seed mass data from my continuous data homework
...
## aggregated.data is now the most clean data
...
#claimed non-ultrametric, so fixed branch lengths    
...
#new.p Branch length issues:
new.phy <- ape::chronoMPL(new.p)
print(ape::is.ultrametric(new.phy))
plot(new.p$edge.length, new.phy$edge.length, xlab="Original tree", ylab="Post-chronos tree", pch=20, bty="n")
...
#problem with bifurcating in tree
...
##to get the states.sd for QuaSSE
states.sd<-names(seed.sd.vector) %in% new.phy$tip.label
states.sd.data<-seed.sd.vector[!(states.sd)]

#Quasse
library(diversitree)

stuff<-geiger::treedata(new.phy, cleaned.vector)
sd.estimates <- stuff$data[,1]
sd.estimates <- median(states.sd.data)
get.stuff<-starting.point.quasse(stuff$phy,stuff$data[,1], sd.estimates)

lambda <- function(x) sigmoid.x(x, 0.1, 0.2,  0, 4) #4 because of default
mu <- function(x) constant.x(x, 0.03)
char <- make.brownian.with.drift(0, 0.025)


quasse.ml <- make.quasse(lambda, mu, tree=stuff$phy, states = stuff$data[,1], states.sd = sd.estimates)
##mle<- find.mle(quasse.ml, get.stuff, method="optim")
## this is the part that I am still struggling with. Will continue to work on it.
```
