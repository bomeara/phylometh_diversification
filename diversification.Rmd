...
depth.range <- range(unlist(lapply(yule.trees,ape::branching.times)), unlist(lapply(bd.trees,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
plot(x=c(0, -4), y=c(20, ape::Ntip(yule.trees[[1]])), log="y", type="n", bty="n", xlab="Time", ylab="N")
colors=c(rgb(1,0,0,0.5), rgb(0, 0, 0, 0.5))
list.of.both <- list(bd.trees, yule.trees)
for (i in sequence(2)) {
    tree.list <- list.of.both[[i]]
...
```{r, eval=TRUE}
my.trees <- TreeSim::sim.bd.taxa(n=30, numbsim=10, lambda=1, mu=0.05, complete=FALSE)
...
#Mangrove slug tree and discrete traits

NSG_Tree<- read.tree("mangrove_treeFinal2.tre")
data1 <- read.csv(file="MangroveDiscrete.csv", stringsAsFactors=FALSE, header=FALSE)

mangrove.vector<-data1[,2]
names(mangrove.vector)<-data1[,1]
name.check(NSG_Tree, mangrove.vector)

#my version of making sure names match data :)

library(corHMM)
cor_out<-corHMM(NSG_Tree,data1,rate.cat=2,node.states="none") #incorrect number of dimensions? why?
BDR<-birthdeath(NSG_Tree)

turnover.anc<- c(1,1,2,2)
eps.anc<- c(1,1,2,2)

trans_rate<-cor_out$solution
trans_rate<-ParDrop(trans_rate, c(3,5,8,10))


#is this correct?

Hisse_T_rate<-TransMatMaker(hidden.states=TRUE)
Hisse_T_rate<-ParDrop(Hisse_T_rate, c(3,5,8,10))

mod1<-hisse(NSG_Tree,data1,hidden.states=TRUE,turnover.anc=turnover.anc,eps.anc=eps.anc,trans.rate=trans_rate)

mod2<-hisse(NSG_Tree,data1,hidden.states=TRUE,turnover.anc=turnover.anc,eps.anc=eps.anc,trans.rate=Hisse_T_rate)

#errors in both:

#Error in c(p.new, 0)[pars] : 
